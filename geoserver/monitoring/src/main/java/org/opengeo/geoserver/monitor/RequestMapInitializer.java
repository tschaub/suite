...
public class RequestMapInitializer implements GeoServerInitializer {
    ...
    public void initialize(GeoServer geoServer) throws Exception {
        ...
        if ((ws = cat.getWorkspaceByName("monitor")) == null) {
            ws = cat.getFactory().createWorkspace();
            ws.setName("monitor");
            cat.add(ws);
                
            NamespaceInfo ns = cat.getFactory().createNamespace();
            ns.setPrefix("monitor");
            ns.setURI("http://monitor.opengeo.org");
            cat.add(ns);
        }
        
        //set up the styles
        addStyle(cat, "monitor_world");
        addStyle(cat, "monitor_requests_agg");
        ...
        if (cat.getDataStoreByName("monitor", "monitor_requests") == null) {
            DataStoreInfo ds = cat.getFactory().createDataStore();
            ds.setWorkspace(ws);
            ds.setName("monitor_requests");
            ...
            FeatureTypeInfo ft = cat.getFactory().createFeatureType();
            ft.setName("monitor_requests_agg");
            ft.setNativeName("monitor_requests_agg");
            ft.setNamespace(cat.getNamespaceByPrefix(ws.getName()));
            ft.setStore(ds);
            ft.setEnabled(true);
            setWorldBounds(ft);
            
            VirtualTable vt = new VirtualTable("monitor_requests_agg", 
            ...
            addLayer(cat, ft, "monitor_requests_agg");
            
            ft = cat.getFactory().createFeatureType();
            ft.setName("monitor_requests");
            ft.setNativeName("monitor_requests");
            ft.setNamespace(cat.getNamespaceByPrefix(ws.getName()));
            ft.setStore(ds);
            ft.setEnabled(true);
            setWorldBounds(ft);
            
            vt = new VirtualTable("monitor_requests", 
                "SELECT ST_SetSRID(ST_MakePoint(remote_lon,remote_lat), 4326) as point," +
